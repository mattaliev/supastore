...
class Query(graphene.ObjectType):
    ...
    order_get_by_id = graphene.Field(OrderType, order_id=graphene.UUID())
    order_get_by_cart_id = graphene.Field(OrderType, cart_id=graphene.UUID())

    def resolve_orders(self, info, **kwargs):
        orders = Order.objects.all()
        return orders

    def resolve_order_get_by_id(self, info, order_id):
        logger = logging.getLogger(self.__class__.__name__)
    ...
    def resolve_order_get_by_cart_id(self, info, cart_id):
        logger = logging.getLogger(self.__class__.__name__)
