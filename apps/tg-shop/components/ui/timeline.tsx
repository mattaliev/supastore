...
const timelineItemVariants = cva("grid items-center gap-x-2", {
  variants: {
    status: {
      done: "text-telegram-text-color",
      default: "text-telegram-hint-color"
    }
  },
  defaultVariants: {
    status: "default"
  ...
  "col-start-2 col-end-3 row-start-1 row-end-1 flex size-4 items-center justify-center rounded-full border border-current",
  {
    variants: {
      status: {
        default: "[&>*]:hidden",
        current:
          "[&>*:not(.lucide-circle)]:hidden [&>.lucide-circle]:fill-telegram-hint-color [&>.lucide-circle]:text-telegram-hint-color",
        done: "bg-telegram-button-color [&>*:not(.lucide-check)]:hidden [&>.lucide-check]:text-background",
        error:
...
const timelineContentVariants = cva(
  "row-start-2 row-end-2 pb-4 text-muted-foreground",
  {
    variants: {
      side: {
...
const timelineHeadingVariants = cva(
  "row-start-1 row-end-1 line-clamp-1 max-w-full truncate",
  {
    variants: {
      ...
      variant: {
        primary: "text-base font-medium text-telegram-button-color",
        secondary: "text-base font-medium text-telegram-hint-color"
      }
    },
    defaultVariants: {
      side: "right",
    ...
    return (
      <hr
        ...
        className={cn(
          ...
          done ? "bg-telegram-button-color" : "bg-telegram-hint-color",
          className
        )}
        ref={ref}
        {...props}
